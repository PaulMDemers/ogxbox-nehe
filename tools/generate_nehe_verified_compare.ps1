...
function Get-ImageComparison {
    ...
    try {
        ...
        return [ordered]@{
            ...
            expected_lit_samples = $expectedLit
            actual_lit_samples = $actualLit
...
function Get-LessonPaths {
    ...
    $num = "{0:D2}" -f $Lesson
    return [ordered]@{
        windows = Join-Path $captureRoot "windows\nehe_ref_$num.png"
...
function New-LessonSheet {
    ...
    $labels = @("Windows reference", "NXGL xemu", "PBKit xemu")
    $images = @($paths.windows, $paths.nxgl, $paths.pb)
    $pad = 12
    $titleHeight = 28
    $labelHeight = 22
    $sheet = New-Object System.Drawing.Bitmap(($Width * 3 + $pad * 4), ($Height + $titleHeight + $labelHeight + $pad * 3))
...
try {
    ...
    foreach ($lesson in $Lessons) {
        ...
        $lessonStats = [ordered]@{
            lesson = $lesson
            windows = Get-ImageStats $paths.windows
            nxgl = Get-ImageStats $paths.nxgl
            pb = Get-ImageStats $paths.pb
            nxgl_vs_windows = Get-ImageComparison $paths.windows $paths.nxgl
            pb_vs_windows = Get-ImageComparison $paths.windows $paths.pb
        }
        ...
        $sheets.Add($sheet)
        $sheet.Save((Join-Path $outDir ("nehe_verified_compare_{0:D2}.png" -f $lesson)), [System.Drawing.Imaging.ImageFormat]::Png)
    }

    Save-Grid $sheets 2 (Join-Path $outDir "nehe_verified_compare_all.png")
    ...
    $manifestPath = Join-Path $outDir "nehe_verified_compare_metrics.json"
    ...
    if ($Strict -and $manifest.warnings.Count -gt 0) {
        throw "Verified comparison generated warnings:`n$($manifest.warnings -join "`n")"
    }
} finally {
    $fontTitle.Dispose()
    $fontSmall.Dispose()
...
Write-Host "Updated verified NeHe comparison sheets in $outDir"
